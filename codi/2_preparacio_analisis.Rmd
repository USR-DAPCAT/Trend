...
calcul_p_trend<-function(dt=dades_qualis,var="Q4") {
  ...
  result<-
   ...
   dplyr::distinct(tibble(variable=lab,p_valor=result$p,statistic=result$statistic,method=result$method))
...
calcul_p_trend_mean<-function(dt=dades_numeriques,var="num1") {
  ...
  tibble(variable=lab,p_valor=PP2$`Pr(F)`[1],F_Value=PP2$`F value`[1],Method="Anova 1 Factor")
...
plot_t_trend<-function(dt=dades_numeriques,variable="num4") {
  ...
  pvalor<-calcul_p_trend_mean(dt,variable) %>% pull(p_valor) %>% round(4)
  
  etiqueta_var<-dt %>% filter(tipo==variable) %>% pull(NAMES)
  
  dt2 %>% ggplot(aes(any,val))  + geom_line() + geom_point() + 
    ggtitle(paste0(etiqueta_var, " durant 2007-2018")) + 
